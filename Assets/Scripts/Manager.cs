...
public class Manager : MonoBehaviour
{
    ...
    private int GetFirstActivePlayer()
    {
        for (int i = 0; i < PlayerInGame.Length; i++)
        {
            if (PlayerInGame[i])
            {
                return i + 1; // Les joueurs sont index�s � partir de 1
            }
        }
        return -1; // G�rer le cas o� tous les joueurs sont �limin�s
    }

    // Jouer un son d'attaque al�atoire
    ...
    public void AffichageGagnant()
    {
        ...
        menu_event.text = "Partie termin�e !";
        Player1Gold.text = "";
        Player2Gold.text = "";
        Player3Gold.text = "";
        Player4Gold.text = "";
    ...
    // D�cr�mentation du nbre de carte apr�s en avoir perdu une
    ...
    // add gold a un joueur sp�cifique
    ...
    void AddDamage(float percAmount, CardAttack target, int currentPlayer)
    {
        TMPro.TextMeshProUGUI[] targetStats = target.GetComponentsInChildren<TMPro.TextMeshProUGUI>();

        if (targetStats.Length == 0)
        {
            Debug.LogError("Aucun TextMeshProUGUI trouvé sur la carte ciblée.");
            return;
        }

        int targetAtk;
        if (!int.TryParse(targetStats[0].text, out targetAtk))
        {
            Debug.LogError("Impossible de convertir l'attaque de la carte en entier.");
            return;
        }

        double bonus = targetAtk * percAmount;
        targetAtk += (int)bonus;
        targetStats[0].text = targetAtk.ToString();
    ...
    public void EndRound()
    {
        ...
        if (currentPlayer == startingPlayerForPhase)
        {
            ...
            else if (currentPhase == 2)
            {
                ...
                {
                    int index = Random.Range(1, PlayerInGame.Length + 1);
                    DisplayEvent($"Buff de damage pour chaque carte du joueur {index} !");
                    GameObject currentDeck = null;
                    switch (index)
                    {
                        case 1:
                            currentDeck = DeckPlayer1;
                            break;
                        case 2:
                            currentDeck = DeckPlayer2;
                            break;
                        case 3:
                            currentDeck = DeckPlayer3;
                            break;
                        case 4:
                            currentDeck = DeckPlayer4;
                            break;
                        default:
                            return;
                    }
                    if (currentDeck == null)
                    {
                        Debug.LogError("Deck introuvable pour le joueur " + index);
                        return;
                    }
                    int cardCount = currentDeck.transform.childCount;
                    foreach (Transform card in currentDeck.transform)
                    {
                        CardAttack cardAttack = card.GetComponent<CardAttack>();
                        if (cardAttack != null)
                        {
                            AddDamage(1.2f, cardAttack, index);
                        }
                    }
                }
            }
